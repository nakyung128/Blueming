...
class HomeViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    @IBOutlet var emotionResult: UIImageView!
    @IBOutlet var healthResult: UIImageView!
    ...
    override func viewDidLoad() {
        ...
        if UserDefaults.standard.string(forKey: "date") != nil {
            if let lastSavedDate = UserDefaults.standard.string(forKey: "date"), lastSavedDate != todayString {
                ...
                emotionResult.isHidden = true
                healthResult.isHidden = true
                ...
                if emotionKeyword != nil && healthKeyword == nil {
                    emotionResult.isHidden = false
                    healthResult.isHidden = true
                    emotionResult.image = UIImage(named: emotionImg!)
    ...
    @objc func handleEmotionUpdate() {
        ...
        if emotionKeyword != nil && healthKeyword != nil {
            ...
            case "감정기복", "두려움", "불안/초조", "스트레스", "슬픔": todayGoals.append(Goals(date: todayString, img: "checklist1.png", title: "나를 위한 격려 해주기", script: "나를 위한 따뜻한 격려를 해 보세요!\n충분히 잘 하고 있으니까요.", first: false, second: false, third: false))
            case "죄책감", "혼란", "무기력": todayGoals.append(Goals(date: todayString, img: "checklist6.png", title: "가벼운 운동 하기", script: "가벼운 홈트레이닝도 좋아요.\n지친 몸을 풀어 봅시다!", first: false, second: nil, third: nil))
            case "예민/짜증", "우울": todayGoals.append(Goals(date: todayString, img: "checklist7.png", title: "산책하기", script: "잠깐이라도 광합성은 필수!\n산책 나가 볼까요?", first: false, second: nil, third: nil))
            case "분노", "절망감": todayGoals.append(Goals(date: todayString, img: "checklist8.png", title: "명상 시간 가지기", script: "잠시나마 여유를 챙기며\n스스로의 마음을 살펴 보아요.", first: false, second: nil, third: nil))
            default:
                return
            }
        ...
        if let goals = UserDefaults.standard.goals(forKey: "goalsDataKey") {
            ...
            if firstGoal.second != nil {
                ...
                if count1 == 0 && more1 == 3 {
                    ...
                    let newAlert = Alert(date: todayString, title: "'\(secondGoal.title)'를 아직 완료하지 못했어요", script: "체크리스트를 완성하고 내 생활을 알차게 채워봐요!", isRead: false, vc: "checkVC")
                    ...
                    if count1 == 2 {
                        // 알람 생성하기
                        let newAlert = Alert(date: todayString, title: "'\(firstGoal.title)' 완료까지 1번 남았어요", script: "마지막 한 번까지 파이팅! :)", isRead: false, vc: "checkVC")
                    ...
                    let newAlert = Alert(date: todayString, title: "'\(firstGoal.title)'를 완성했어요!", script: "정말 대단해요!", isRead: false, vc: "checkVC")
                ...
                if firstGoal.first {
                    ...
                    goalCnt1.attributedText = NSAttributedString(string: "도전하기!", attributes: [NSAttributedString.Key.font: UIFont(name: "Pretendard-Bold", size: 14)!, NSAttributedString.Key.kern: -0.7])
                    moreCnt1.attributedText = NSAttributedString(string: "\(more1)번 해 보세요!", attributes: [NSAttributedString.Key.font: UIFont(name: "Pretendard-Bold", size: 10)!, NSAttributedString.Key.kern: -0.5])
                    
                    let newAlert = Alert(date: todayString, title: "'\(firstGoal.title)'를 아직 완료하지 못했어요", script: "체크리스트를 완성하고 내 생활을 알차게 채워봐요!", isRead: false, vc: "checkVC")
            ...
            if secondGoal.second != nil {
                ...
                if count2 == 0 && more2 == 3 {
                    ...
                    if count2 == 2 {
                        let newAlert = Alert(date: todayString, title: "'\(secondGoal.title)' 완료까지 1번 남았어요", script: "마지막 한 번까지 파이팅! :)", isRead: false, vc: "checkVC")
                    ...
                    let newAlert = Alert(date: todayString, title: "'\(secondGoal.title)'를 완성했어요!", script: "정말 대단해요!", isRead: false, vc: "checkVC")
                ...
                if secondGoal.first {
                    ...
                    goalCnt2.attributedText = NSAttributedString(string: "도전하기!", attributes: [NSAttributedString.Key.font: UIFont(name: "Pretendard-Bold", size: 14)!, NSAttributedString.Key.kern: -0.7])
                    moreCnt2.attributedText = NSAttributedString(string: "\(more2)번 해 보세요!", attributes: [NSAttributedString.Key.font: UIFont(name: "Pretendard-Bold", size: 10)!, NSAttributedString.Key.kern: -0.5])
